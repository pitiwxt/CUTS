...
\begin{center}
  ...
  \hrule
  \vskip 0.4cm
  % --- Prototype Access Block (First Page) ---
  \begin{minipage}{0.44\textwidth}
    \centering
    \qrcode[height=2.8cm]{https://zocialtrackerprototype.vercel.app}\\[0.2cm]
    {\small\bfseries\color{chulapink} Live Demo — Club OS}\\[0.05cm]
    {\footnotesize\url{https://zocialtrackerprototype.vercel.app}}
  \end{minipage}
  \hfill
  \begin{minipage}{0.44\textwidth}
    \centering
    \qrcode[height=2.8cm]{https://github.com/pitiwxt/CUTS}\\[0.2cm]
    {\small\bfseries\color{darkblue} Source Code — GitHub}\\[0.05cm]
    {\footnotesize\url{https://github.com/pitiwxt/CUTS}}
  \end{minipage}
  \vskip 0.3cm
  \hrule
\end{center}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.75\textwidth]{"raplace-diagram/Screenshot 2569-06-25 at 20.01.03.png"}
...
\begin{enumerate}[label=\bfseries สูตรที่ \arabic*:]
  \item \textbf{Engagement Rate (ER) [อัตราเอนเกจ]}: วัดคุณภาพความน่าสนใจของคอนเทนต์เทียบกับยอดคนที่เห็นจริง งานวิจัยของ Keib et al. (2018) [2] พบว่ายอดเอนเกจ (Engage) เป็นตัวบ่งชี้ความเติบโตของช่องทางที่แม่นยำกว่ายอดติดตามดิบถึง 3.2 เท่า
    \begin{equation}
...
\begin{itemize}
  \item \textbf{สำหรับฝ่ายประชาสัมพันธ์ (Creator)}: ทำงานตามปกติ ไม่ต้องลงตารางสรุปยอดสถิติเอง เพียงโพสต์คลิมหรือรูป แล้วระบุรหัสแคมเปญ (เช่น \#Recruiter-2026) ลงในแคปชันหรือชื่อแชร์ของโพสต์
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.95\textwidth]{"raplace-diagram/Screenshot 2569-06-25 at 18.59.03.png"}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.75\textwidth]{"raplace-diagram/Screenshot 2569-06-25 at 19.30.50.png"}
...
\noindent เมื่อระบบนำยอดผู้เข้าชมและข้อมูลรายจ่ายมาประสานเข้าด้วยกัน จะสามารถตอบคำถามสำคัญ ทำให้การบริหารงานโดยรวมของชมรมสะดวกขึ้น \emph{"งบประมาณที่ชมรมใช้ไป แลกมาด้วยคนเห็นกี่คน และได้เอนเกจ (Engage) ละกี่บาท"}
...
\begin{highlightbox}
  ...
  \begin{equation}
    \text{Cost per Reach (CPR)} = \frac{\text{ยอดเงินใช้จ่ายรวมในโครงการ (จาก OCRchat)}}{\text{ยอดจำนวนการเข้าถึงรวม (จาก Zocial Tracker)}}
  \end{equation}
  \begin{equation}
    \text{Cost per Engagement (CPE)} = \frac{\text{ยอดเงินใช้จ่ายรวมในโครงการ (จาก OCRchat)}}{\text{ยอดเอนเกจรวม (จาก Zocial Tracker)}}
  \end{equation}
\end{highlightbox}

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.75\textwidth]{"raplace-diagram/Screenshot 2569-06-25 at 18.59.55.png"}
...
\begin{itemize}
  \item \textbf{แคมเปญรับสมัครสมาชิกใหม่ (Recruiter-2026)}: ยอดใช้จ่ายยิงแอด 1,950 บาท แลกกับยอด Reach 22,700 ครั้ง และยอดเอนเกจ 3,630 ครั้ง บนระบบ Dashboard จะแสดงให้ประธานเห็นว่ามี CPR เท่ากับ 0.08 บาทต่อ Reach และ CPE เท่ากับ 0.53 บาทต่อ Engagement ซึ่งเป็นสถิติที่คุ้มค่ามากสำหรับการรับสมัครสมาชิกใหม่
...
\section{Web Prototype: ระบบที่พัฒนาและ Deploy จริงแล้ว — Club OS v2.0}

\noindent เพื่อพิสูจน์ว่าแนวคิดในเอกสารนี้สามารถนำมาสร้างได้จริง ผู้จัดทำได้พัฒนา \textbf{Web Prototype} ที่ทำงานได้จริงบน Production Environment โดยใช้ Next.js 16, Supabase, Groq AI และ Vercel Deploy ครบวงจร สามารถเข้าใช้งานได้ทันทีโดยไม่ต้อง Login

\subsection{ช่องทางการเข้าถึง Prototype}

\begin{figure}[h!]
\centering
\begin{minipage}{0.45\textwidth}
  \centering
  \qrcode[height=3.5cm]{https://zocialtrackerprototype.vercel.app}\\[0.3cm]
  {\small\bfseries\color{chulapink} Live Demo: Club OS}\\[0.1cm]
  {\footnotesize\url{https://zocialtrackerprototype.vercel.app}}
\end{minipage}
\hfill
\begin{minipage}{0.45\textwidth}
  \centering
  \qrcode[height=3.5cm]{https://github.com/pitiwxt/CUTS}\\[0.3cm]
  {\small\bfseries\color{darkblue} Source Code: GitHub}\\[0.1cm]
  {\footnotesize\url{https://github.com/pitiwxt/CUTS}}
\end{minipage}
\caption{\chulafont QR Code สำหรับเข้าถึง Web Prototype และ Source Code บน GitHub}
\end{figure}

\begin{table}[h!]
\centering
\caption{\chulafont Tech Stack ที่ใช้ใน Prototype จริง}
\begin{tabular}{lll}
\toprule
\textbf{Layer} & \textbf{Technology} & \textbf{หน้าที่} \\
\midrule
Frontend & Next.js 16 + Tailwind CSS & Dashboard UI + SSR \\
AI Vision (OCR) & Groq \texttt{llama-4-scout-17b} & สแกนใบเสร็จ \\
AI Chat & Google Gemini & วิเคราะห์ Campaign \\
Database & Supabase PostgreSQL & จัดเก็บข้อมูลถาวร \\
Sheets Sync & Google Apps Script & Export รายจ่ายอัตโนมัติ \\
...
\subsection{Module 1: Zocial Tracker — Marketing Dashboard}

\noindent เมื่อเปิดเว็บจะพบหน้าแรก Club OS ให้เลือก Module จากนั้นเข้าสู่ระบบ Zocial Tracker ที่แสดง KPI Dashboard พร้อมสถิติจริงจาก Google Sheets และ Supabase

\begin{figure}[h!]
\centering
\begin{subfigure}[b]{0.48\textwidth}
  \centering
  \includegraphics[width=\textwidth]{prototype-screenshots/01-home.png}
  \caption{\chulafont หน้าแรก Club OS — เลือก Module}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
  \centering
  \includegraphics[width=\textwidth]{prototype-screenshots/02-overview.png}
  \caption{\chulafont Overview Dashboard — KPI Cards + Trend Chart}
\end{subfigure}
\caption{\chulafont Club OS Home และ Zocial Tracker Overview}
\end{figure}
...
\begin{subfigure}[b]{0.48\textwidth}
  \centering
  \includegraphics[width=\textwidth]{prototype-screenshots/03-performance.png}
  \caption{\chulafont Performance — Trend Chart Reach/Engagement}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
  \centering
  \includegraphics[width=\textwidth]{prototype-screenshots/04-pipeline.png}
  \caption{\chulafont Pipeline Status — 4-Tier Acquisition System}
\end{subfigure}
\caption{\chulafont Performance Trend และ Acquisition Pipeline}
\end{figure}
...
\begin{subfigure}[b]{0.48\textwidth}
  \centering
  \includegraphics[width=\textwidth]{prototype-screenshots/05-datatable.png}
  \caption{\chulafont Data Table — Post Database พร้อม Filter}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
  \centering
  \includegraphics[width=\textwidth]{prototype-screenshots/06-chat.png}
  \caption{\chulafont AI Analyst Assistant — วิเคราะห์ Campaign ROI}
\end{subfigure}
\caption{\chulafont Data Table และ AI Chat Assistant}
\end{figure}

\subsection{Module 2: OCRchat — Finance \& Expense Tracking}

\noindent ระบบ OCRchat ในโมดูล Finance รับรูปใบเสร็จผ่าน Drag \& Drop แล้วส่งให้ Groq Llama 4 Scout วิเคราะห์และแกะข้อมูลออกมาใส่ฟอร์มอัตโนมัติ จากนั้นบันทึกลง Supabase และ Google Sheets พร้อมกัน

\begin{figure}[h!]
\centering
\includegraphics[width=0.80\textwidth]{prototype-screenshots/07-ocrchat-upload.png}
\caption{\chulafont OCRchat — Drag \& Drop Upload Zone พร้อมฟอร์มบันทึกรายจ่ายด้วย Groq Llama 4 Scout}
\end{figure}

\subsection{ผลลัพธ์ที่วัดได้จาก Prototype จริง}

\begin{tcolorbox}[colback=chulapink!3, colframe=chulapink!30, title=\chulafont\bfseries สรุปสิ่งที่ Prototype ทำงานได้จริงแล้ว, breakable]
\begin{itemize}
  \item \textbf{Zocial Tracker}: ดึงข้อมูลจาก Google Sheets จริง แสดงผล KPI, Trend Chart, Platform breakdown พร้อม Tier 3 Apify scraper ทำงานได้จริงผ่านปุ่ม
  \item \textbf{OCRchat}: สแกนใบเสร็จธนาคาร (YouTrip/KBank) แกะยอดเงิน 710 บาท, วันที่, ประเภท transport ได้ถูกต้อง บันทึกลง Supabase และ Google Sheets tab ใหม่ \texttt{CUTS-Club-OS} อัตโนมัติ
  \item \textbf{AI Assistant}: ตอบคำถามภาษาไทยเกี่ยวกับ Campaign วิเคราะห์ ER, VS และ CPR ได้
  \item \textbf{Export}: ปุ่ม Export CSV ดึงข้อมูลรายจ่ายทั้งหมดจาก Supabase มาเป็นไฟล์ได้ทันที
  \item \textbf{Security}: ไม่มี API Key hardcode ในโค้ด ทุก secret ใช้ Vercel Environment Variables
\end{itemize}
\end{tcolorbox}
