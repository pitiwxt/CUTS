...
import { LayoutDashboard, BarChart3, GitFork, Table, Bot, Receipt, ChevronLeft, TrendingUp, X } from 'lucide-react';
...
export default function Sidebar({ currentTab, setCurrentTab, module, setModule, isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside className={`
        w-64 bg-white border-r border-slate-200 flex flex-col fixed h-full z-50 shadow-sm
      ...
      `}>
        ...
        <div className="p-5 border-b border-slate-200">
          ...
          <button
            onClick={() => handleNav(() => setModule('home'))}
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          {module === 'home' && (
            <>
              ...
              <button
                onClick={() => handleNav(() => { setModule('zocial'); setCurrentTab('overview'); })}
              ...
              <button
                onClick={() => handleNav(() => setModule('ocrchat'))}
            ...
            <>
              ...
              {zocialItems.map(item => {
                ...
                return (
                  <button
                    key={item.id}
                    onClick={() => handleNav(() => setCurrentTab(item.id))}
